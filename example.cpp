...
int main()
{
  ...
  log.with_field(log.custom_field={{"animal", "walrus"}, {"total", 100}}).info("INFO of WALRUS and its total");
  log.info("No Walrus");
  log.with_field(log.custom_field={{"animal", "dog"}, {"total", float_test}}).warn("WARN of DOG and its total");
  log.warn("No Dog");

  
  log.JSON_output(); //output in JSON format
  log.with_field(log.custom_field={{"animal", "cat"}, {"total", long_test}}).critical("CRITICAL of CAT and its total");
  log.critical("No Cat");
  log.with_field(log.custom_field={{"animal", "falcon"}, {"total", 10}}).error("ERROR of FALCON and its total");
  log.error("No Falcon");

  auto staticformat = log.with_field(log.custom_field={{"float",float_test}, {"long", long_test}, {"string", a}});
  ...
  log.default_output();
  auto staticformat2 = log.with_field(log.custom_field={{"animal","griffin"}, {"rider", "azure"}, {"weapon", "spear"}});
